let handler = async (m, { q, conn, isOwner, setReply }) => {
  try {
    ...
    try {
      ...
      if (!m.isBotAdmin) return setReply(mess.only.Badmin);
      setReply(typeof err === 'string' ? err : err && err.message ? `Gagal mengeluarkan: ${err.message}` : JSON.stringify(err, null, 2));
    }
  } catch (e) {
    console.error('[KICKME ERROR]:', e);
    setReply('Terjadi error saat menjalankan perintah');
  }
};
handler.help = ["kickme"];
handler.tags = ["admin"];
