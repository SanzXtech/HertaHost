...
try {
  ...
  await conn.sendMessage(m.chat, { text: `Gagal bergabung ke grup: ${err.message || err}. Pastikan tautan undangan valid atau owner memverifikasi bot.` }, { quoted: m })
}

await add(id, subject, LinkGc, Second)

let groupMetadata = nana == undefined ? {} : await conn.groupMetadata(id).catch(_ => ({}))
